...
#         "--ground_truth",
#         type=str,
#         default="bns_layman_eval_dataset.json",
#         help="Path to ground truth JSON file (default: bns_layman_eval_dataset.json)"
#     )
#     parser.add_argument(
#         "--output",
#         type=str,
...
def evaluate_single_query(query: str, relevant_sections: list):
    """
    Run retrieval for one query and return Hit Rate@5 + RR.
    """
    # Step 1: Retrieval only (no reformulation, no LLM analysis)
    results = search_sections(query, top_k=MAX_K)
...
def run_evaluation(ground_truth_path: str, output_path: str):
    ...
    for idx, item in enumerate(ground_truth, start=1):
        ...
        start   = time.time()
        result  = evaluate_single_query(query, relevant_sections)
        elapsed = round(time.time() - start, 2)
